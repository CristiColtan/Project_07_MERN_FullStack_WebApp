...
export default function CreateListing() {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      ...
      const res = await fetch(
        `http://localhost:8081/backend/listing/get/${listingID}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:8081/backend/listing/update/${params.listingID}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="main-create-listing">
      <div>
        <br></br>
        <h3>Update your property informations!</h3>
        <p>Edit the form if you want any changes!</p>
        <br></br>
        <Form>
          <Row>
            <Col>
